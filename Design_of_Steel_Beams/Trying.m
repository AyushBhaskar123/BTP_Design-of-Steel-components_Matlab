function varargout = Trying(varargin)
% TRYING MATLAB code for Trying.fig
%      TRYING, by itself, creates a new TRYING or raises the existing
%      singleton*.
%
%      H = TRYING returns the handle to a new TRYING or the handle to
%      the existing singleton*.
%
%      TRYING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRYING.M with the given input arguments.
%
%      TRYING('Property','Value',...) creates a new TRYING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Trying_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Trying_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Trying

% Last Modified by GUIDE v2.5 08-Sep-2019 18:39:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Trying_OpeningFcn, ...
                   'gui_OutputFcn',  @Trying_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Trying is made visible.
function Trying_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Trying (see VARARGIN)

% Choose default command line output for Trying
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Trying wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Trying_OutputFcn(hObject, eventdata, handles) 
...
if(i==1)
    plot([0 len],[0 0],'LineWidth',5,'Color','k')
    text((len/3),-1,'Beam','Color','k','FontSize',14)
    xlim([0 len]);
    ylim([-5 5]);
    hold on;
    ...
    setappdata(0,'V_matrix_value',V);
    setappdata(0,'M_matrix_value',M);
...
if(i==2)   
    ...
    ylim([-(p/2) (p/2)]);
    str1=['P=' num2str(p) ' KN'];
    text(a,(p/4),str1,'Color','black','Fontsize',14)
    ...
    z1=(p*(len-a)*x1/(6*len)).*((len^2)-((len-a)^2)-(x1.^2));
    z2=(p*a*(len-x2)/(6*len)).*((2*len*x2)-(x2.^2)-(a^2));
    ...
    y2=zeros(1,(((len-a)/0.1)));

    y3=(R1*x1)-(M1);
    y4=zeros(1,(((len-a)/0.1)));             
    end

    v_shear_point=[y1 y2];
    m_bending_point=[y3 y4];
    z_point=[z1 z2];
...
if(i==3)
    ...
    str2=['w=' num2str(w) ' KN/m'];
    text((a+b)/2,w/4,str2,'Color','black','Fontsize',14);
    
    c1=a:0.01:b;
    c2=sin((w/5)*c1);
    plot(c1,abs(c2),'Color','blue','LineWidth',3);
    ...
    C5=(-R2*((b-len)^2)*(len+(2*b))/(6*len))+(w*((b-a)^3)*(a+(3*b))/(24*len))-(R1*(b^3)/(3*len));
    C6=-C5*len;
    C1=(C5)+(R2*((b-len)^2)/2)-(w*((b-a)^3)/6)+(R1*(b^2)/2);
    
    z3=(-R1*(x3.^3)/6)+(C1*x3);
    z4=(w*((x4-a).^4)/24)-(R1*((x4).^3)/6)+(C1*x4);
    z5=(R2*((x5-len).^3)/6)+(C5*x5)+(C6);
    ...
    if(radio2==1)
        ...
        C5=(M1*b)-(R1*(b^2)/2)+(w*((b-a)^3)/6);
        C6=(-M1*(b^2)/2)+(R1*(b^3)/3)+(w*((b-a)^4)/24)-(w*b*((b-a)^3)/6);
        
        z3=(M1*(x3.^2)/2)-(R1*(x3.^3)/6);
        z4=(M1*(x4.^2)/2)-(R1*(x4.^3)/6)+(w*((x4-a).^4)/24);
        z5=(C5*x5)+(C6);
        ...
        y7=zeros(1,(((len-b)/0.1)));
        
        y8=(R1*x3)-(M1);
        y9=(R1*x4)-(M1)-((w*(x4-a).*(x4-a))/2);
        y10=zeros(1,(((len-b)/0.1)));
    ...
    v_shear_udl=[y5 y6 y7];
    m_bending_udl=[y8 y9 y10];
    z_udl=[z3 z4 z5];
...
if(i==5)
    w1=getappdata(0,'w1value');
    w2=getappdata(0,'w2value');
    a=getappdata(0,'avalue');
    b=getappdata(0,'bvalue');
    
    str2=['W1=' num2str(w1) ' KN/m'];
    str3=['W2=' num2str(w2) ' KN/m'];
    text(a,w1/4.5,str2,'Color','black','Fontsize',14);
    text(b,w2/3.5,str3,'Color','black','Fontsize',14);
    
    drawArrow = @(x,y) quiver( x(1),y(1),x(2)-x(1),y(2)-y(1),0,'Color','black','LineWidth',3,'MaxHeadSize',1);    
    x_arrow = [a a];
    y_arrow = [(w1/5) 0];
    drawArrow(x_arrow,y_arrow);
    hold on;
    
    plot([a b],[w1/5 w2/5],'Color','black','LineWidth',3);
    
    x_arrow1 = [b b];
    y_arrow1 = [(w2/5) 0];
    drawArrow(x_arrow1,y_arrow1);
    ...
    if(radio1==1)
    R2=((b-a)/(6*len))*((w1*(2*a+b))+(w2*(2*b+a)));
    R1=((w1+w2)*(b-a)/2)-R2;
    
    C5=(R2*((b-len)^3)/(6*len))-(R1*(b^3)/(3*len))-(R2*((b-len)^2)*b/(2*len))+(w1*((b-a)^3)*b/(6*len))-(w1*((b-a)^4)/(24*len))-((w2-w1)*((b-a)^4)*b/(120*len));
    C6=-C5*len;
    C1=C5+(R2*((b-len)^2)/2)+(R1*(b^2)/2)-(w1*((b-a)^3)/6)-((w2-w1)*((b-a)^3)/24);
    
    z6=(-R1*(x3.^3)/6)+(C1*x3);
    z7=(-R1*(x4.^3)/6)+(w1*((x4-a).^4)/24)+((w2-w1)*((x4-a).^5)/(120*(b-a)))+(C1*x4);
    z8=(R2*((x5-len).^3)/6)+(C5*x5)+(C6);
    
    y11=ones(1,((a/0.1)+1));
    y11=y11*R1;
    
    y12=R1-(w1*(x4-a))-(0.5*(w2-w1)*((x4-a).*(x4-a))/(b-a));
    
    y13=ones(1,((len-b)/0.1));
    y13=y13*(-R2);
    
    y14=R1*x3;
    y15=(R1*x4)-(w1*(x4-a).*(x4-a)/2)-((1/6)*(w2-w1)*(x4-a).*(x4-a).*(x4-a)/(b-a));
    y16=R2*(len-x5);
    end
    ...
    if(radio2==1)
        ...
        R1=((w1+w2)*(b-a)/2);
        M1=((b-a)/6)*((w1*(2*a+b))+(w2*(a+2*b)));
        
        C5=(M1*b)-(R1*(b^2)/2)+(w1*((b-a)^3)/6)+((w2-w1)*((b-a)^3)/24);
        C6=(-M1*(b^2)/2)+(R1*(b^3)/3)+(w1*((b-a)^4)/24)-(w1*b*((b-a)^3)/6)+((w2-w1)*((b-a)^4)/120)-((w2-w1)*((b-a)^3)*b/24);

        z6=(M1*(x3.^2)/2)-(R1*(x3.^3)/6);
        z7=(M1*(x4.^2)/2)-(R1*(x4.^3)/6)+(w1*((x4-a).^4)/24)+((w2-w1)*((x4-a).^5)/(120*(b-a)));
        z8=(C5*x5)+(C6);
    ...
    v_shear_udl=[y11 y12 y13];
    m_bending_udl=[y14 y15 y16];
    z_uvl=[z6 z7 z8];
...
if(i==4)
    ...
    str2=['W1=' num2str(w1) ' KN/m'];
    str3=['W2=' num2str(w2) ' KN/m'];
    text(a-1,w1/3.5,str2,'Color','black','Fontsize',14);
    text(b,w2/4.5,str3,'Color','black','Fontsize',14);
...
len=getappdata(0,'lenvalue');
A=getappdata(0,'V_matrix_value');
B=getappdata(0,'M_matrix_value');
Z=getappdata(0,'Z_matrix_value');

maxsf=max(max(A),-min(A));
maxbm=max(max(B),-min(B));
Zi=max(max(Z),-min(Z));
if(radio1==1)
    Za=len*1000/300;
end
