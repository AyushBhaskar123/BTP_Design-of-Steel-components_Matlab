function varargout = CantileverUDL(varargin)
% CANTILEVERUDL MATLAB code for CantileverUDL.fig
%      CANTILEVERUDL, by itself, creates a new CANTILEVERUDL or raises the existing
%      singleton*.
%
%      H = CANTILEVERUDL returns the handle to a new CANTILEVERUDL or the handle to
%      the existing singleton*.
%
%      CANTILEVERUDL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CANTILEVERUDL.M with the given input arguments.
%
%      CANTILEVERUDL('Property','Value',...) creates a new CANTILEVERUDL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CantileverUDL_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CantileverUDL_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CantileverUDL

% Last Modified by GUIDE v2.5 19-Aug-2019 21:27:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CantileverUDL_OpeningFcn, ...
                   'gui_OutputFcn',  @CantileverUDL_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CantileverUDL is made visible.
function CantileverUDL_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CantileverUDL (see VARARGIN)

% Choose default command line output for CantileverUDL
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes CantileverUDL wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CantileverUDL_OutputFcn(hObject, eventdata, handles) 
...
s3=sprintf("Enter the intensity of Uniformly Distributed Load acting on the beam in KN");
set(handles.edit2,'TooltipString',s3);

s4=sprintf("Enter the starting point of UDL from Left Support in metres");
set(handles.edit3,'TooltipString',s4);

s5=sprintf("Enter the ending point of UDL from Left Support in metres");
set(handles.edit4,'TooltipString',s5);

s6=sprintf("Click to get the Shear Force Diagram of the Given Loading Conditions");
set(handles.pushbutton3,'TooltipString',s6);

s7=sprintf("Click to get the Bending Moment Diagram of the Given Loading Conditions");
set(handles.pushbutton4,'TooltipString',s7);
...
y4=(R1*x1)-(M1);
y5=(R1*x2)-(M1)-((w*(x2-a).*(x2-a))/2);
y6=zeros(1,(((len-b)/0.1)+1));
