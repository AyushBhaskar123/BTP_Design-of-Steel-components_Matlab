function varargout = SimplyUDL(varargin)
% SIMPLYUDL MATLAB code for SimplyUDL.fig
%      SIMPLYUDL, by itself, creates a new SIMPLYUDL or raises the existing
%      singleton*.
%
%      H = SIMPLYUDL returns the handle to a new SIMPLYUDL or the handle to
%      the existing singleton*.
%
%      SIMPLYUDL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SIMPLYUDL.M with the given input arguments.
%
%      SIMPLYUDL('Property','Value',...) creates a new SIMPLYUDL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SimplyUDL_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SimplyUDL_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SimplyUDL

% Last Modified by GUIDE v2.5 19-Aug-2019 21:24:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SimplyUDL_OpeningFcn, ...
                   'gui_OutputFcn',  @SimplyUDL_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SimplyUDL is made visible.
function SimplyUDL_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SimplyUDL (see VARARGIN)

% Choose default command line output for SimplyUDL
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SimplyUDL wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SimplyUDL_OutputFcn(hObject, eventdata, handles) 
...
s3=sprintf('Enter the intensity of Uniformly Distributed Load acting on the beam in KN/m');
...
y1=y1*R1;
y2=R1-(w*(x2-a));
y3=ones(1,(((len-b)/0.1)+1));
y3=y3*(-R2);
A=[y1 y2 y3];
maxsf=max(max(A),-min(A));

y4=R1*x1;
y5=(R1*x2)-(w*(x2-a).*(x2-a)/2);
y6=R2*(len-x3);
B=[y4 y5 y6];
