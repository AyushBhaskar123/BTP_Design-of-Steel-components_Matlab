function varargout = CantileverPointUdl(varargin)
% CANTILEVERPOINTUDL MATLAB code for CantileverPointUdl.fig
%      CANTILEVERPOINTUDL, by itself, creates a new CANTILEVERPOINTUDL or raises the existing
%      singleton*.
%
%      H = CANTILEVERPOINTUDL returns the handle to a new CANTILEVERPOINTUDL or the handle to
%      the existing singleton*.
%
%      CANTILEVERPOINTUDL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CANTILEVERPOINTUDL.M with the given input arguments.
%
%      CANTILEVERPOINTUDL('Property','Value',...) creates a new CANTILEVERPOINTUDL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CantileverPointUdl_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CantileverPointUdl_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CantileverPointUdl

% Last Modified by GUIDE v2.5 19-Aug-2019 23:02:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CantileverPointUdl_OpeningFcn, ...
                   'gui_OutputFcn',  @CantileverPointUdl_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CantileverPointUdl is made visible.
function CantileverPointUdl_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CantileverPointUdl (see VARARGIN)

% Choose default command line output for CantileverPointUdl
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes CantileverPointUdl wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CantileverPointUdl_OutputFcn(hObject, eventdata, handles) 
...
s7=sprintf("Enter the intensity of Uniformly Distributed Load in KN/m");
set(handles.edit4,'TooltipString',s7);
...
s8=sprintf("Enter the starting point of UDL from Left Support in metres");
set(handles.edit5,'TooltipString',s8);

s9=sprintf("Enter the ending point of UDL from Left Support in metres");
set(handles.edit6,'TooltipString',s9);
...
if (c<=a) && (c<=b)
    ...
    y5=(R1*x1)-M1;
    y6=(R1*x2)-M1-(p*(x2-c));
    y7=(R1*x3)-M1-(p*(x3-c))-(w*((x3-a).^2)/2);
    y8=zeros(1,(((len-b)/0.1)+1));
...
if (c>=a) && (c<=b)
    ...
    y5=(R1*x1)-M1;
    y6=(R1*x2)-M1-(w*((x2-a).^2)/2);
    y7=(R1*x3)-M1-(w*((x3-a).^2)/2)-(p*(x3-c));
    y8=zeros(1,(((len-b)/0.1)+1));
...
setappdata(0,'cvalue',c);
setappdata(0,'wvalue',w);
setappdata(0,'avalue',a);
setappdata(0,'bvalue',b);
CantileverPointUdlSFD
